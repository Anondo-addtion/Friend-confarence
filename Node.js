...
wss.on('connection', (ws, req) => {
  ...
  // ১. IP ইতিমধ্যেই ব্লকড কি না চেক (রিফ্রেশ করলেও ব্লকই থাকবে)
  ...
  // ২. পাসওয়ার্ড সিকিউরিটি ও ৩০ মিনিট অটো-আনব্লক লজিক
  if (!password || password !== CHAT_PASSWORD) {
    failedAttempts[clientIP] = (failedAttempts[clientIP] || 0) + 1;

    if (failedAttempts[clientIP] >= 3) {
      blockedIPs.add(clientIP);
      console.log(`IP Blocked: ${clientIP} for 30 minutes`);
      ws.close(4003, 'IP_BLOCKED');

      // ⏳ ৩০ মিনিট পর স্বয়ংক্রিয়ভাবে IP আনব্লক হবে
      setTimeout(() => {
        blockedIPs.delete(clientIP);
        failedAttempts[clientIP] = 0;
        console.log(`IP Automatically Unblocked: ${clientIP}`);
      }, BLOCK_DURATION);
  ...
  // পাসওয়ার্ড সঠিক হলে ব্যর্থ চেষ্টার সংখ্যা রিসেট হবে
  failedAttempts[clientIP] = 0;
