   // -------------------------------------------------------------
    ...
    async function startConnection() {
      ...
      socket.addEventListener('message', async (event) => {
        try {
          ...
          if (data.system) {
            // পাসওয়ার্ড সঠিক প্রমাণিত হলে তবেই চ্যাটরুমে এক্সেস পাবে
    ...
    // Keepalive Ping-Pong Mechanic (২০ সেকেন্ডের ব্যবধানে)
    ...
    function appendChatMessage(sender, parsed, isSelf = false) {
      ...
      if (parsed.type === 'text') {
        ...
        contentHtml = `<a href="${parsed.data}" target="_blank" class="chat-link">
          <i class="fa-solid fa-map-location-dot"></i> 📍 মাই লোকেশন দেখুন
        </a>`;
      } else if (parsed.type === 'file_link') {
        contentHtml = `<a href="${parsed.data.url}" target="_blank" download class="chat-link">
          <i class="fa-solid fa-file-arrow-down"></i> 📁 ${escapeHtml(parsed.data.fileName)} (ডাউনলোড)
        </a>`;
      }
